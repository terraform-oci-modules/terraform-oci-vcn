...
When `create_database_subnet_route_table = true`, database subnets are associated with their own route table containing NAT Gateway and Service Gateway routes, while private subnets retain their own separate NAT route table(s). All subnets are **regional** (`availability_domains` not set) - each spans all availability domains automatically.
