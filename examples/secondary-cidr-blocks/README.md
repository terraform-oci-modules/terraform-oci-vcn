...
OCI supports up to five CIDR blocks per VCN. Subnets can be allocated from any attached CIDR - they are not restricted to the primary range. All subnets are **regional** (`availability_domains` not set) - each spans all availability domains automatically.
