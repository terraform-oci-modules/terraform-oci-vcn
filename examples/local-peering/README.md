...
After peering, each VCN's route table must have a route pointing the remote CIDR at its own LPG. This example uses the symbolic `"lpg@<key>"` notation in `internet_gateway_route_rules` and `nat_gateway_route_rules`. Subnets in both VCNs are **regional** (`availability_domains` not set) - each spans all availability domains automatically.
